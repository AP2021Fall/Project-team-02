...
public class ScoreBoardController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
        ScoreBoardResponse scoreBoardResponse = controller.showScoreBoard(UserInfo.getSelectedTeam());

        if (scoreBoardResponse.isSuccessful()){
            List<String> response =  new ArrayList<>();
            scoreBoardResponse.getUsersScores().forEach((username, score) ->response.add(username +" : " + score));
            ObservableList<String> observableArrayList =
                    FXCollections.observableArrayList(response);

            logsListView.setItems(observableArrayList);
        }else{
            JOptionPane.showMessageDialog(null,scoreBoardResponse.getMessage());
